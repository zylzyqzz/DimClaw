...
pub fn merge_config(base: &mut PluginConfigFile, updates: &PluginConfigFile) {
    if !updates.app_id.is_empty() {
        base.app_id = updates.app_id.clone();
    }
    if !updates.app_secret.is_empty() {
        base.app_secret = updates.app_secret.clone();
    }
    if !updates.verify_token.is_empty() {
        base.verify_token = updates.verify_token.clone();
    }
